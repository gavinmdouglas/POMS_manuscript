...
PZ_OPTIONS <- options_manager(env_column = 'env', dset_column = 'dataset', which_envir = 'env1', prior_type = "uninformative", sample_column = 'sample')


meta_table <- test.abd.meta$metadata
focal_var_level <- "env1"
var_colname <- "env"
sample_colname <- "sample"
silence_citation = FALSE


abun_table <- as.matrix(test.abd.meta$mtx)
taxa_labels <- rownames(abun_table)
abun_table <- apply(abun_table, 2, as.integer)
rownames(abun_table) <- taxa_labels

tmp <- prevalence_norm_logit(abun_table = abun_table,
                             meta_table = test.abd.meta$metadata,
                             focal_var_level = "env1",
                             var_colname = "env",
                             sample_colname = "sample")

tmp2 <- prev.addw(test.abd.meta)

identical(tmp, tmp2)



ess_orig <- calc.ess(test.abd.meta)

ess_new <- specificity_scores(abun_table = abun_table,
                              meta_table = test.abd.meta$metadata,
                              focal_var_level = "env1",
                              var_colname = "env",
                              sample_colname = "sample")
...
prevalence_norm_logit <- function(abun_table,
  ...
  if (! silence_citation) {
    ...
    message("If you use these normalized prevalence values, please cite the original authors: https://doi.org/10.1371/journal.pcbi.1006242.")
  }
...
specificity_scores <- function(abun_table,
  ...
  if (! silence_citation) {
    ...
    message("If you use these values, please cite the original authors: https://doi.org/10.1371/journal.pcbi.1006242.")
  }
...
calc.ess_TEST <- function(abd.meta,
                     pdata = NULL,
                     b.optim = NULL,
                     ...) {
  
  abd.meta <- test.abd.meta
  pdata = NULL
  b.optim = NULL
