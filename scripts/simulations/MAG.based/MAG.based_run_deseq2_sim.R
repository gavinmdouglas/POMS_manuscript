...
unperturbed_alt.tools <- mclapply(1:1000, function(rep_i) {
  ...
  alt_tools_out <- run_alt.tools(func_abun_table = almeida_func_abun,
                                 ...
                                 tools_to_run = c("deseq2"))
  
  alt_tools_out[["func"]] <- "unperturbed"
  
  saveRDS(object = alt_tools_out,
          file = paste("DA_tool_out/deseq2_output/unperturbed/MAG.based_unperturbed_deseq2_", as.character(rep_i), "reps.rds", sep = ""))
...
func_sim_alt.tools <- mclapply(X = 1:1000, FUN = function(rep_i) {
  ...
  saveRDS(object = output,
          file = paste("DA_tool_out/deseq2_output/func.based/MAG.based_func.based_deseq2_", as.character(rep_i), "reps.rds", sep = ""))
...
taxa_sim_alt.tools <- mclapply(X = 1:1000, FUN = function(rep_i) {
  ...
  saveRDS(object = output,
          file = paste("DA_tool_out/deseq2_output/taxa.based/MAG.based_taxa.based_deseq2_", as.character(rep_i), "reps.rds", sep = ""))
...
clade.based_alt.tools <- mclapply(X = 1:693, FUN = function(rep_i) {
  ...
  saveRDS(object = output,
          file = paste("DA_tool_out/deseq2_output/clade.based/MAG.based_clade.based_deseq2_", as.character(rep_i), "reps.rds", sep = ""))
