...
mclapply(X = 1:693, FUN = function(rep_i) {
  
  in_rds <- paste("MAG.based_prepped_clade.based_sim_info_sel1.5/", "clade.based_sim_info_rep", as.character(rep_i), ".rds", sep = "")
  
  rep_clade.based_sim_info <- readRDS(in_rds)
  
  rep_func_abun <- calc_func_abun(in_abun = rep_clade.based_sim_info$taxa_perturb_abun, in_func = almeida_func_subset, ncores = 1)
  
  outfile <- paste("func_abun_tables_clade.based_sim_sel1.5/", "func_abun_tab_rep", as.character(rep_i), ".rds", sep = "")
